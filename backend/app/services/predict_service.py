...
class PredictService:
    ...
    async def run_prediction(
        ...
        try:
            ...
            ab_variant, ab_artifact_path, ab_test_id = await ab_svc.select_variant_for_prediction()
        except Exception:
            ab_variant = None
        ...
        if ab_variant is not None and ab_artifact_path is not None:
            ...
            model_version = f"{ab_variant}:{ab_artifact_path.split('/')[-1].replace('.pkl', '')}"
        else:
        ...
        for record_dict, prob in zip(record_dicts, probabilities, strict=False):
        ...
        PREDICTION_LATENCY.labels(model_version=clean_version).observe(time.monotonic() - start)
        ...
        for result in results:
            ...
            CHURN_PROBABILITY_HISTOGRAM.labels(model_version=clean_version).observe(result.churn_probability)
