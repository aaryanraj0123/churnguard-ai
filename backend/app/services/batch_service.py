...
class BatchService:
    ...
    async def upload_csv(
        ...
        if file.content_type not in ALLOWED_CONTENT_TYPES:
            raise InvalidFileException(f"Invalid MIME type: {file.content_type}. Expected text/csv")

        # Read and validate size
        content = await file.read()
        if len(content) > settings.max_upload_size_bytes:
            raise FileTooLargeException(f"File exceeds {settings.max_upload_size_mb}MB limit")
