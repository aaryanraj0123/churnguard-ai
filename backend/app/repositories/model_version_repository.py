...
class ModelVersionRepository:
    ...
    async def get_active(self) -> ModelVersion | None:
        result = await self._db.execute(select(ModelVersion).where(ModelVersion.is_active.is_(True)))
        return result.scalar_one_or_none()

    async def get_by_id(self, version_id: uuid.UUID) -> ModelVersion | None:
        result = await self._db.execute(select(ModelVersion).where(ModelVersion.id == version_id))
        return result.scalar_one_or_none()

    async def get_by_tag(self, version_tag: str) -> ModelVersion | None:
        result = await self._db.execute(select(ModelVersion).where(ModelVersion.version_tag == version_tag))
        return result.scalar_one_or_none()

    async def list_all(self) -> list[ModelVersion]:
        result = await self._db.execute(select(ModelVersion).order_by(ModelVersion.created_at.desc()))
