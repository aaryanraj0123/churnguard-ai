...
class PredictionJobRepository:
    ...
    async def get_by_id(self, job_id: uuid.UUID) -> PredictionJob | None:
        result = await self._db.execute(select(PredictionJob).where(PredictionJob.id == job_id))
        return result.scalar_one_or_none()

    async def get_by_id_and_user(self, job_id: uuid.UUID, user_id: uuid.UUID) -> PredictionJob | None:
    ...
    async def list_by_user(
        ...
        total_result = await self._db.execute(select(func.count()).select_from(base_query.subquery()))
    ...
    async def set_completed(self, job_id: uuid.UUID, result_path: str, processed_count: int) -> None:
        job = await self.get_by_id(job_id)
        if job:
