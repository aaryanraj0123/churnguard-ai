...
class DriftService:
    ...
    async def run_drift_check(
        ...
        for fr in report.feature_results:
            ...
            if fr.drift_detected:
                DRIFT_ALERTS_COUNTER.labels(feature=fr.feature, severity=fr.severity).inc()
    ...
    async def get_latest_report(self, model_version_tag: str | None = None) -> DriftReportResponse | None:
        report = await self._drift_repo.get_latest(model_version_tag)
        if report is None:
            return None
        return DriftReportResponse.model_validate(report)

    async def list_reports(self, page: int = 1, page_size: int = 20) -> list[DriftReportResponse]:
