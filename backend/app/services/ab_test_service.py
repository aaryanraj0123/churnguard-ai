...
class ABTestService:
    ...
    async def stop_test(self, test_id: uuid.UUID) -> ABTestStopResponse:
        ...
        # Compute winner
        ctrl_mean = test.control_churn_sum / test.control_requests if test.control_requests > 0 else None
        trt_mean = test.treatment_churn_sum / test.treatment_requests if test.treatment_requests > 0 else None
    ...
    async def record_ab_prediction(
        ...
        AB_TEST_REQUEST_COUNTER.labels(experiment_name=str(test_id), variant=variant).inc()
        AB_TEST_CHURN_RATE_GAUGE.labels(experiment_name=str(test_id), variant=variant).set(
            mean_churn_probability
        )

    def _to_response(self, test: ABTest) -> ABTestResponse:
        ctrl_mean = (
            round(test.control_churn_sum / test.control_requests, 6) if test.control_requests > 0 else None
        )
        trt_mean = (
            round(test.treatment_churn_sum / test.treatment_requests, 6)
            if test.treatment_requests > 0
            else None
        )
