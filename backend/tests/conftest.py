...
# ── Test database ─────────────────────────────────────────────────────────────
# In CI, DATABASE_URL already points to churnguard_test. Locally it points to
# churnguard — derive _test only when the DB name doesn't already end in _test.
_db_url = settings.database_url
if not _db_url.endswith("/churnguard_test"):
    TEST_DATABASE_URL = _db_url.rsplit("/", 1)[0] + "/churnguard_test"
else:
