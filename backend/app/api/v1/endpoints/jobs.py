...
async def list_jobs(
    ...
    return await service.list_user_jobs(user_id=current_user.id, page=page, page_size=page_size)
...
async def get_job_status(
    ...
    service = BatchService(db)
    result = await service.get_job_status(job_id=job_id, user_id=current_user.id, is_admin=is_admin)
...
async def download_results(
    ...
    is_admin = current_user.role == UserRole.ADMIN
    service = BatchService(db)
    job = await service.get_job_status(job_id=job_id, user_id=current_user.id, is_admin=is_admin)
