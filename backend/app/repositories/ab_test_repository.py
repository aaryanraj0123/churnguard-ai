...
class ABTestRepository:
    ...
    async def get_active(self) -> ABTest | None:
        result = await self._db.execute(
            select(ABTest).where(ABTest.is_active.is_(True)).order_by(ABTest.started_at.desc()).limit(1)
        )
        return result.scalar_one_or_none()

    async def get_by_name(self, name: str) -> ABTest | None:
        result = await self._db.execute(select(ABTest).where(ABTest.name == name))
        return result.scalar_one_or_none()

    async def get_by_id(self, test_id: uuid.UUID) -> ABTest | None:
        result = await self._db.execute(select(ABTest).where(ABTest.id == test_id))
        return result.scalar_one_or_none()

    async def list_all(self) -> list[ABTest]:
        result = await self._db.execute(select(ABTest).order_by(ABTest.created_at.desc()))
